...
#define MY_PORT    "30570"    // my port number for UDP connection
...
void SetUpUDP() {
  ...
  if (p == NULL) {
    fprintf(stderr, "hospitalA: failed to create socket\n");
    exit(1);
  }
...
void PrintBootupMsg() {
  printf("\nHospital A is up and running using UDP on port %s.\n", MY_PORT);
  printf("Hospital A has: \n");
...
void Dijkstra(int src) {
  ...
  if (rebase.find(src) != rebase.end()) {
    ...
    rebased_src = -1;
    printf("Panic - hospitalA location not in map!!!\n");
    exit(1);
  }

  bool seen[rebase_counter];
  // memory allocated for num elements of rows
...
void SendTo(double d1, double d2) {
  ...
  if (numbytes == -1) {
      perror("hospitalA: sendto");
      exit(1);
  }
}

void CalculateScore() {
  // my_loc is hospitalA location
  int client_loc = (int) buf[1];

  // Invalid location -- not in map
  if (rebase.find(client_loc) == rebase.end()) {
    printf("Hospital A does not have the location %i in map\n", client_loc);
  ...
  if (rebased_client_loc == rebase.at(my_loc)) {
    printf("Hospital A is at same location as client\n");
    SendTo(0, 0); // return to scheduler immediately
    printf("Hospital A has sent distance = \"None\" to the Scheduler\n");
  }

  double distance = dist[rebased_client_loc];
  double availability = (cap - occ) / (double) cap;
  printf("Hospital A has capacity = %i, occupation = %i, ", cap, occ);
  ...
  printf("Hospital A has sent to the Scheduler: \n");
  if (score != 0) {
    printf("\tscore = %f\n", score);
  } else {
    printf("\tscore = \"None\"\n");
...
void RecvInfo() {
  ...
  if (buf[0] == 0) { // Receive request for score and distance from scheduler
    printf("\n\nHospital A has received input from client at location %i\n", 
           (int) buf[1]);
    CalculateScore();
  } else if (buf[0] == 1) { // Receive confirmation of client assignment
    ++occ;
    double availability = (cap - occ) / (double) cap;
    printf("Hospital A has been assigned to a client: \n");
    printf("\toccupation is updated to %i\n", occ);
    printf("\tavailability is updated to %f\n", availability);
  } else {
    printf("Panic: hospitalA received an unknown message from the Scheduler\n");
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    fprintf(stderr,"usage: hospitalA <location A> ");
    fprintf(stderr, "<capacity A> <initial occupancy A>\n");
    exit(1);
  }
  ...
  Dijkstra(my_loc); // Calcualte shortest paths from hospitalA
  PrintBootupMsg();
