...
#define MY_PORT    "31570"    // my port number for UDP connection
...
void SetUpUDP() {
  ...
  if (p == NULL) {
    fprintf(stderr, "hospitalB: failed to create socket\n");
    exit(1);
  }
...
void PrintBootupMsg() {
  printf("\nHospital B is up and running using UDP on port %s.\n", MY_PORT);
  printf("Hospital B has: \n");
...
void Dijkstra(int src) {
  ...
  if (rebase.find(src) != rebase.end()) {
    ...
    rebased_src = -1;
    printf("Panic - hospitalB location not in map!!!\n");
    exit(1);
  }
...
void SendTo(double d1, double d2) {
  ...
  if (numbytes == -1) {
      perror("hospitalB: sendto");
      exit(1);
  }
}

void CalculateScore() {
  // my_loc is hospitalB location
  int client_loc = (int) buf[1];

  // Invalid location -- not in map
  if (rebase.find(client_loc) == rebase.end()) {
    printf("Hospital B does not have the location %i in map\n", client_loc);
  ...
  if (rebased_client_loc == rebase.at(my_loc)) {
    printf("Hospital B is at same location as client\n");
    SendTo(0, 0); // return to scheduler immediately
    printf("Hospital B has sent distance = \"None\" to the Scheduler\n");
  }

  double distance = dist[rebased_client_loc];
  double availability = (cap - occ) / (double) cap;
  printf("Hospital B has capacity = %i, occupation = %i, ", cap, occ);
  ...
  printf("Hospital B has sent to the Scheduler: \n");
  if (score != 0) {
    printf("\tscore = %f\n", score);
  } else {
    printf("\tscore = \"None\"\n");
...
void RecvInfo() {
  ...
  if (buf[0] == 0) { // Receive request for score and distance from scheduler
    printf("\n\nHospital B has received input from client at location %i\n", 
           (int) buf[1]);
    CalculateScore();
  } else if (buf[0] == 1) { // Receive confirmation of client assignment
    ++occ;
    double availability = (cap - occ) / (double) cap;
    printf("Hospital B has been assigned to a client: \n");
    printf("\toccupation is updated to %i\n", occ);
    printf("\tavailability is updated to %f\n", availability);
  } else {
    printf("Panic: hospitalB received an unknown message from the Scheduler\n");
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    fprintf(stderr,"usage: hospitalB <location B> ");
    fprintf(stderr, "<capacity B> <initial occupancy B>\n");
    exit(1);
  }
  ...
  Dijkstra(my_loc); // Calcualte shortest paths from hospitalB
  PrintBootupMsg();
